...
`power.nb` is a package developed for estimating statistical power and sample sizes in differential abundance microbiome studies. The package presents novel methods for estimating statistical power for individual taxa (feature, otu, or species)  and for c sample size calculation accounting for effect sizes and mean abundances of taxa in a differential abundance study. 

Data simulation is required in order to estimate statistical power and to estimate sample sizes required to detect a target statistical power. `power.nb` therefore presents with two novel microbiome data simulations frameworks: `MixGassSim` and `RRSim`. The method implemented in `MixGassSim` involves fitting a mixture of  Mixture of Gaussian Distributions to the distributions of mean abundance of taxa and fold changes and simulates count data from the negative binomial model implemented in the  `DESeq2` R package. Details about the method implemented in `MixGassSim` presented in the paper ["Investigating statistical power of differential abundance studies"](https://doi.org/10.1371/journal.pone.0318820).  The method implemented in `RRSim` built upon the functionality of the `glmmTMB` package and simulated microbiome data from a Mixed Effect Model with accounting for correlations between taxa and zero inflation in microbiome data. 
...
We would need to simulate microbiome data in order to estimate statistical power at the level of individual taxon.  The simulation framework implemented in `power.nb` is Mixture of Gaussian Mixture Model proposed in the paper by [Agronah and Bolker](https://doi.org/10.1371/journal.pone.0318820). 
...
In other to estimate statistical power for individual power, we need to simulate data that resembles actual microbiome data. Here we use the `MixGaussSim`  simulation method. 


## Two ways to obtain parameters for data simulation (MixGaussSim)
There are one of two ways to obtain the parameters of the Mixture of Gaussian Mixture Model for data simulation.  Users can:

**1. pre-specify a set of parameters:** To help users decide on plausible parameter values, we have run  `MixGaussSim` on some of the microbiome datasets used in the paper “Microbiome differential abundance methods produce different results across 38 datasets.”[@Nearing2022] and presented the parameter estimates  obtained from each of these datasets ([see Appendix](#appendix) for the parameter estimates from these datasets). 

**2. estimate parameters from an existing dataset:** Secondly, users can also obtain estimates directly from fitting the model implemented in  `MixGaussSim`  to exiting datasets. The following steps outlines the process for estimating parameters if you have a dataset. To ensure that results are applicable to the study you intend conducting. 



## Dataset

In this vignette, we are using a subset of the Artic Fire datasets; one of the datasets presented in the paper by Nearing. The data has 148 samples and 31100 taxa. We are using the subset just to illustrate how `MixGaussSim` works and for faster running. 
