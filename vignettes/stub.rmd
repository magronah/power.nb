...
```{r logfoldchange, message=FALSE, cache = TRUE}
...
```{r logmean, message=FALSE, warning=FALSE, cache = TRUE}
...
```{r contour-plot, eval=TRUE, cache = TRUE}
...
```{r sim_multi_ss, eval = TRUE, cache = TRUE}
nsim = 10
...
```{r deseq_multi_ss, eval = TRUE, cache = TRUE}
desq_est_list  =  list()
...
We now fit a GAM model to predict statistical power. This time, we predict GAM with covariates as mean count of taxa, fold change of taxa and sample sizes

```{r gam_multi_ss, eval = FALSE, cache = TRUE}
true_logfoldchange
true_logmean
logfoldchange_list
true_logmean

power_multi_ss <- power_fun_ss(deseq_est_list,
                         true_logfoldchange,
                         true_logmean,
                         sample_vec,
...
i want to use latex format for mu1 and mu2 instead. Ie, mu_1
...
df <- data.frame(
  ...
  `$n_{comp}$` = c(5, 5),
  `$\\mu_1$` = c("(2,3,4,5,7)", "(2,3,4,5,7)"),
  `$\\sigma_1$` = c("(2,3,4,5,7)", "(2,3,4,5,7)"),
  `$p_1$` = c("(2,3,4,5,7)", "(2,3,4,5,7)"),
  `$n_{comp}$` = c(5, 5),
...
We also sincerely appreciate the following individuals for running the functions in this package on the microbiome datasets published in [@Nearing2022], thereby providing users with plausible parameter values for their research work: Alim Tuguzbay, Arjya Bhattacharjee, Atiyah Sulaiman, George Vinichenko, Josh Effero, Mahdiya Adnan, Mir Akbaar Khaled, Nevin Xu, Nichole, Riya, and Shaira Habib (University of Alberta: Alim Tuguzbay, Arjya Bhattacharjee, Atiyah Sulaiman, George Vinichenko, Josh Effero, Mahdiya Adnan, Mir Akbaar Khaled, Nevin Xu, and Shaira Habib; McMaster University: Nichole and Riya).

<!--We also sincerely appreciate the following people for running the functions in this package on the microbiome datasets published in [@Nearing2022] to provide users with plausible parameter values for thier research work: Alim Tuguzbay, Nevin Nu, Mir Akbaar Khaled, Gosh, logfoldchangeFit, Alim, Akja, etc. and Roda (all from University of Alberta) and mmm (McMaster University)-->
